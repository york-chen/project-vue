    ...
    error => {
        ...
        if (error.response && (error.response.status === 401 || error.response.status === 403)) {
            ...
            if (
                ...
                toast('登录过期或账号权限不足，请重新登录！')
            }
        } else if (error.response.status === 404) {
            toast('服务器搬家啦')
        } else if (error.response.status === 500) {
            toast('后台程序出错了，请联系管理员')
...
function errorState(response, showToast) {
    ...
    if (response && (response.status === 200 || response.status === 304 || response.status === 400)) {
        !showToast && toast(response.data.errors || response.data.msg || response.data.message || '网络异常')
...
function successState(res) {
    ...
    if (res.data && (res.data.code === 200 || res.data.code === 9100)) {
        ...
        res.data === 'object' && toast(res.data.errors || res.data.msg || res.data.message || '网络异常')
    }
}
